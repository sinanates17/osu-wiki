...
- **If multiple playstyles of a single key mode are present in a single beatmap, the playstyle must be denoted using a recognisable marker in all difficulty names of the key mode (e.g. "14K EZ2AC Easy" and "14K DP Easy"). Otherwise, it is optional to denote the playstyle.** `+`
...
- **Beatmaps using the special N+2 style of mapping must include "N+2" in the tags. For beatmaps containing multiple key modes, difficulties using this style must be denoted using either using N+2 notation (e.g. using "10K+2" instead of "12K" on difficulty names), OR using a recognisable playstyle marker (e.g. "12K DP" instead of "12K")** `+`
