...
- **Beatmaps using the special N+2 style of mapping must include "N+2" in the tags. For beatmaps containing multiple key modes, difficulties using this style must be denoted using either using N+2 notation (e.g. using 10K+2 instead of 12K on difficulty names), OR a recognizable descriptor that matches the specific playstyle (e.g. using 16K DP rather than 14K+2).** `+`
